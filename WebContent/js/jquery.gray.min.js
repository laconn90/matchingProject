...
window.Modernizr = window.Modernizr || function (a, b, c) {
  ...
  return j.addTest = function (a, b) {
    if ('object' == typeof a) for (var d in a) z(a, d) && j.addTest(d, a[d]);
     else {
...
Modernizr.addTest('cssfilters', function () {
  ...
  return a.style.cssText = Modernizr._prefixes.join('filter:blur(2px); '),
  !!a.style.length && (void 0 === document.documentMode || document.documentMode > 9)
}),
...
function (a, b, c, d) {
  function g(b, c) {
    ...
    g = d.fade || f.classes.fade,
    c.fade = c.fade || b.className.indexOf(g) > - 1,
    this.element = b,
  ...
  a.extend(g.prototype, {
    ...
    extractUrl: function (a) {
      ...
      endRegex = /["']?\)$/,
      b = a.replace(startRegex, '').replace(endRegex, '')
    },
    positionToNegativeMargin: function (a) {
      var b,
      c,
      d;
      return b = a.match(/^(-?\d+\S+)/) [0],
      c = a.match(/\s(-?\d+\S+)$/) [0],
      d = 'margin:' + c + ' 0 0 ' + b
    },
    getBgSize: function (b, c) {
      ...
      if (d = new Image, d.src = b, 'auto' !== c && 'cover' !== c && 'contain' !== c && 'inherit' !== c) {
        ...
        e = d.width / d.height,
        g = parseInt((c.match(/^(\d+)px/) || [0,
        0]) [1]),
        i = parseInt((c.match(/\s(\d+)px$/) || [0,
        0]) [1]),
        f = k.height() * e,
        h = k.width() / e,
        g = g || f,
    ...
    getBgParams: function (b) {
      ...
      return d = this.extractUrl(b.css('background-image')),
      bgSize = this.getBgSize(d, b.css('background-size')),
      offset = this.positionToNegativeMargin(b.css('background-position')),
      c.styles = this.getComputedStyle(b[0]),
      c.svg = a.extend({
        url: d
      }, bgSize, {
        offset: offset
    ...
    setStyles: function (a, b, c, d) {
      return b.display = 'inline-block',
      b.overflow = b['overflow-x'] = b['overflow-y'] = 'hidden',
      b['background-image'] = 'url("' + c.url + '")',
      b['background-size'] = d.width + 'px ' + d.height + 'px',
      'Img' === a && (b['background-repeat'] = 'no-repeat', b['background-position'] = c.padding.left + 'px ' + c.padding.top + 'px', b.width = c.width, b.height = c.height),
      delete b.filter,
      b
    },
    addSVGFilterOnce: function () {
      $body = a('body'),
      $body.data('plugin_' + e + '_has_filter') || $body.data('plugin_' + e + '_has_filter', 'true').append('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="0" height="0" style="position:absolute"><defs><filter id="gray"><feColorMatrix type="saturate" values="0"/></filter></defs></svg>')
    },
    switchImage: function (b) {
      ...
      e,
      f;
      c = this.elementType(b),
      d = this['get' + c + 'Params'](b),
      e = this.settings.fade ? this.settings.classes.fade : '',
      f = a('<div class="grayscale grayscale-replaced ' + e + '"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + d.svg.width + ' ' + d.svg.height + '" width="' + d.svg.width + '" height="' + d.svg.height + '" style="' + d.svg.offset + '"><image filter="url(&quot;#gray&quot;)" x="0" y="0" width="' + d.image.width + '" height="' + d.image.height + '" preserveAspectRatio="none" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="' + d.svg.url + '" /></svg></div>'),
      d.styles = this.setStyles(c, d.styles, d.svg, d.image),
      f.css(d.styles),
